import { ProcessedInputs } from "./tokenizer";

export const maskToken = "***mask***";
const cutoffRange = [0.0002, 0.001]

const cleanInput = (userInput: string) => {
...
export function getNoun(nounList: string[], input: string|undefined){
    if(input != undefined){
...
export async function loadModel(tf: any, modelUrl: string){
...
declare const chance: { weighted: (arg0: any[], arg1: number[]) => any; };


export async function generateStat(model: any, input: string|undefined, nounList: string[], 
    processInput: (input: string) => ProcessedInputs, 
    processOutput: (predictions: any, mask: number, noun: string) => [string, number][],
    randomness: number
){
